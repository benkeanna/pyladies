...
import random
puzzle = random.sample(range(1, 10), 4)
number_of_guesses = 0 # pocet pokusu, ktery se bude zsvetsovat po kazdem tahu

class CheckInput: # kontrola, jestli je input 4-mistne cislo s ruznymi cislicemi
    ...
    def check_if_digits_are_different(self): # kontrola, ze input obsahuje 4 ruzne cislice
        number_of_occur = []
        for number in self.input:
            number_of_occur.append(self.input.count(number)) # pocet vyskytu jednotlivych polozek v inputu
        return True if sum(number_of_occur) == len(str(self.input)) else False # suma poctu ma byt stejna jako delka inputu - kazda polozka je v nem jenom jednou


print('Welcome in game cows and bulls. In this game you will have to quess the puzzle made of four different digits.')
while True:
    while True: # overeni validity inputu
    ...
    list_of_bulls = [i for i, j in zip(puzzle, guess) if i == j] # zjisteni poctu bulls
    bulls = len(list_of_bulls)
    print(bulls,'bulls')

    list_of_cows = set(puzzle) & set(guess) # zjiteni poctu cows
    cows = (len(list_of_cows) - bulls)
    print(cows,'cows')
    number_of_guesses += 1

    if bulls == len(puzzle): # vyhodnoceni hry (uzivatel uhadnul)
        break
    else:
        pass
